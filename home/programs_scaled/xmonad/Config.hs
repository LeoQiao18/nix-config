...
main' :: D.Client -> IO ()
main' dbus = xmonad . docks . ewmh . keybindings . urgencyHook . (withNavigation2DConfig def) $ def {
        terminal           = myTerminal,
        focusFollowsMouse  = False,
        clickJustFocuses   = False,
        borderWidth        = 3,
        modMask            = myModMask, -- use 'Super' key
        workspaces         = myWorkspaces,
        normalBorderColor  = "#dddddd", -- light gray
...
myAppLauncher = "rofi -show combi -combi-modi \"window,drun\" -modi \"combi,run,ssh\" -show-icons"
...
myKeys conf@XConfig {XMonad.modMask = modm} =
  keySet "Applications"
    [ key "Slack" (modm, xK_F2) $ spawnOn chatWs "slack"
    ] ^++^
  keySet "Audio"
  ...
  keySet "Brightness"
    [ key "Increase"      (0, xF86XK_MonBrightnessUp        ) $ spawn "light -A 5"
    , key "Decrease"      (0, xF86XK_MonBrightnessDown      ) $ spawn "light -U 5"
    ] ^++^
  keySet "Launchers"
    [ key "Terminal"      (modm                , xK_Return  ) $ spawn (XMonad.terminal conf)
    , key "Apps (Rofi)"   (modm                , xK_p       ) $ spawn myAppLauncher
  ...
  keySet "Windows"
    ...
    , key "Focus left"      (modm              , xK_h        ) $ windowGo L False
    , key "Focus right"     (modm              , xK_l        ) $ windowGo R False
    , key "Focus up"        (modm              , xK_k        ) $ windowGo U False
    , key "Focus down"      (modm              , xK_j        ) $ windowGo D False
    , key "Swap left"       (modm .|. shiftMask, xK_h        ) $ windowSwap L False
    , key "Swap right"      (modm .|. shiftMask, xK_l        ) $ windowSwap R False
    , key "Swap up"         (modm .|. shiftMask, xK_k        ) $ windowSwap U False
    , key "Swap down"       (modm .|. shiftMask, xK_j        ) $ windowSwap D False
  ...
  keySet "Workspaces"
    ...
    , key "Add"           (modm              , xK_o         ) $ addWorkspacePrompt wsPromptTheme
    , key "Remove"        (modm .|. shiftMask, xK_o        ) removeWorkspace
    ] ++ switchWsById
 where
...
myLayout =
  ...
  where
     ...
     column3 = gapSpaced 5 $ ThreeColMid nmaster delta ratio
     ...
     workLayout = onWorkspace workWs (column3 ||| full)
     chatLayout = onWorkspace chatWs (column3 ||| full)
...
myWorkspaces = [devWs, workWs, cfgWs, funWs, chatWs, mediaWs, idleWs]

wsPromptTheme :: XPConfig
wsPromptTheme = amberXPConfig
  { bgHLight = "#002b36"
